...
class TransformDataset(Dataset):
    ...
    def __getitem__(self, id):
        ...
        labels['gt_maps'] = np.round(A.ReplayCompose.replay(
            transform_data['replay'], image=labels['gt_maps'])['image'] / 255)
        labels['eroded_maps'] = np.round(A.ReplayCompose.replay(
            transform_data['replay'], image=labels['eroded_maps'])['image'] / 255)
        return image, labels
