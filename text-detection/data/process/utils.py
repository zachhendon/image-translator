...
def get_maps2(bboxes, size):
    ...
    gt_text = torch.zeros((num_bboxes, *size), dtype=torch.uint8, device="cuda")
    gt_kernel = torch.zeros((num_bboxes, *size), dtype=torch.uint8, device="cuda")
    ...
    gt_kernel = torch.maximum(
        gt_kernel,
        -F.max_pool2d(
            -gt_text.unsqueeze(0).to(dtype=torch.float16),
            9,
            stride=1,
            padding=4,
        )
